...
function doWaitIdlePage(pageActivityObservable, idlePageCallback, maxDuration) {
    ...
    var maxDurationTimeoutId = maxDuration &&
        setTimeout(browser_core_1.monitor(function () { return complete({ hadActivity: true, endClocks: browser_core_1.clocksNow() }); }), maxDuration);
    var pageActivitySubscription = pageActivityObservable.subscribe(function (_a) {
        var isBusy = _a.isBusy;
        clearTimeout(validationTimeoutId);
        clearTimeout(idleTimeoutId);
        var lastChangeTime = browser_core_1.clocksNow();
        if (!isBusy) {
            idleTimeoutId = setTimeout(browser_core_1.monitor(function () { return complete({ hadActivity: true, endClocks: lastChangeTime }); }), exports.PAGE_ACTIVITY_END_DELAY);
        }
    });
    var stop = function () {
        hasCompleted = true;
